...
export class KanbanDesk extends Page{
  ...
  ngAfterViewInit() {     
    ...
    changeColumn(options:MenuCommandItem={},src=null,dst?){        
        let rec=this.id2record(src);
        if(!src){
            return;
        }                             
    ...
    data2menu(data:Array<any>,onlyRoot:boolean,menuName:string,directoryName:string){
